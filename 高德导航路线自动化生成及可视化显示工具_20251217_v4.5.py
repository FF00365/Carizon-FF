        ...
        for i, route in enumerate(routes):
            ...
            if len(locations) >= 2:
                ...
                if highway_distance > 0 or elevated_distance > 0:
                    legend_html += f'''
                    <div id="route-info-{i}" class="route-info" style="margin-left: 20px; font-size: 14px; margin-bottom: 10px;">
...
class MainWindow(QMainWindow):
    ...
    def create_auto_route_tab(self):
        ...
        control_layout.addWidget(self.pause_btn, 14, 1, 1, 2)  # 行14，列1，占1行2列
        ...
        self.route_num_spin.setStyleSheet("font-size: 18px;")
        ...
        self.waypoint_spin.setStyleSheet("font-size: 18px;")
        row1_layout.addWidget(self.waypoint_spin)
        
        # 目标里程设置（非必填）
        row1_layout.addWidget(QLabel("目标里程(km):"))
        self.target_distance_input = QLineEdit()
        self.target_distance_input.setPlaceholderText("选填")
        self.target_distance_input.setFixedWidth(120)
        self.target_distance_input.setFixedHeight(40)
        self.target_distance_input.setStyleSheet("font-size: 18px;")
        row1_layout.addWidget(self.target_distance_input)
        
        # 里程偏差范围（非必填）
        row1_layout.addWidget(QLabel("偏差范围(km):"))
        self.distance_tolerance_input = QLineEdit()
        self.distance_tolerance_input.setPlaceholderText("选填")
        self.distance_tolerance_input.setFixedWidth(120)
        self.distance_tolerance_input.setFixedHeight(40)
        self.distance_tolerance_input.setStyleSheet("font-size: 18px;")
        row1_layout.addWidget(self.distance_tolerance_input)
        ...
        # 相邻途径点距离设置
        row2_layout.addWidget(QLabel("相邻点距离(m):"))
        self.adj_min_input = QLineEdit()
        self.adj_min_input.setPlaceholderText("500")
        self.adj_min_input.setText("500")
        self.adj_min_input.setFixedWidth(80)
        self.adj_min_input.setFixedHeight(40)
        self.adj_min_input.setStyleSheet("font-size: 18px;")
        row2_layout.addWidget(self.adj_min_input)
        row2_layout.addWidget(QLabel("-"))
        self.adj_max_input = QLineEdit()
        self.adj_max_input.setPlaceholderText("1000")
        self.adj_max_input.setText("1000")
        self.adj_max_input.setFixedWidth(80)
        self.adj_max_input.setFixedHeight(40)
        self.adj_max_input.setStyleSheet("font-size: 18px;")
        row2_layout.addWidget(self.adj_max_input)
        
        # 非相邻途径点距离设置
        row2_layout.addWidget(QLabel("非相邻点>(m):"))
        self.non_adj_min_input = QLineEdit()
        self.non_adj_min_input.setPlaceholderText("500")
        self.non_adj_min_input.setText("500")
        self.non_adj_min_input.setFixedWidth(80)
        self.non_adj_min_input.setFixedHeight(40)
        self.non_adj_min_input.setStyleSheet("font-size: 18px;")
        row2_layout.addWidget(self.non_adj_min_input)
        
        # 距离计算方式选择
        row2_layout.addWidget(QLabel("距离计算:"))
        ...
        self.distance_calc_combo.setFixedWidth(240)
        self.distance_calc_combo.setFixedHeight(40)
        self.distance_calc_combo.setStyleSheet("font-size: 18px;")
        row2_layout.addWidget(self.distance_calc_combo)
        
        # # 路线策略选择（已注释）
        # row2_layout.addWidget(QLabel("路线策略:"))
        # self.strategy_combo = QComboBox()
        # self.strategy_combo.addItem("走高速", 34)
        # self.strategy_combo.addItem("不走高速", 35)
        # self.strategy_combo.addItem("大路优先", 37)
        # self.strategy_combo.setFixedWidth(120)
        # self.strategy_combo.currentIndexChanged.connect(self.on_strategy_changed)
        # row2_layout.addWidget(self.strategy_combo)
        ...
        self.generate_route_btn.setFixedHeight(40)
        ...
        # 查看路线按钮（带下拉菜单）
        self.all_routes_button = QPushButton("🗺️ 查看路线")
        self.all_routes_button.setEnabled(False)
        self.all_routes_button.setFixedHeight(40)
        self.all_routes_button.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
        
        # 创建下拉菜单
        from PyQt5.QtWidgets import QMenu
        self.view_routes_menu = QMenu(self)
        self.view_routes_menu.addAction("🗺️ 查看所有路线地图", self.view_all_routes_map)
        self.view_routes_menu.addAction("🔗 查看高德路线规划", self.view_amap_route_links)
        self.all_routes_button.setMenu(self.view_routes_menu)

        # 导出JSON按钮
        self.export_json_btn = QPushButton("💾 导出JSON")
        self.export_json_btn.clicked.connect(self.export_json_files)
        self.export_json_btn.setFixedHeight(40)
        ...
        row3_layout.addWidget(self.generate_route_btn, 1)
        row3_layout.addWidget(self.all_routes_button, 1)
        row3_layout.addWidget(self.export_json_btn, 1)
        row3_layout.addWidget(self.reset_btn, 1)
        route_main_layout.addLayout(row3_layout)
    ...
    def search_scene_locations(self):
        ...
        try:
            ...
            # 获取去重距离（200米）
            dedup_distance = self.route_config.get('dedup_distance', 0.2)
            
            # 在线程中执行搜索
            threading.Thread(
                target=self._search_scene_thread,
                args=(city, selected_districts, selected_scenes, key, dedup_distance),
                daemon=True
            ).start()
    ...
    def _search_scene_thread(self, city, selected_districts, selected_scenes, key, dedup_distance):
        """在线程中搜索场景地点并直接获取坐标"""
        try:
            added_count = 0
            filtered_no_coord = 0
                ...
                for district in selected_districts:
                    ...
                    while True:
                        ...
                        if data.get('status') != '1':
                            self.update_api_response(f"❌ 搜索失败: {data.get('info')}")
                            break
                        ...
                        for poi in pois:
                            ...
                            poi_district = poi.get('adname', '')  # POI所在的行政区
                            ...
                            for existing in self.valid_locations:
                                ...
                                if dist < dedup_distance:
                                    ...
                                    self.update_api_response(f"   ⛔ {name} - 距离{existing['name']}太近({dist*1000:.0f}m<200m)，已过滤")
                                    break
                            ...
                            self.update_api_response(f"   ✅ {name} ({poi_district}) - 坐标: {lon:.6f}, {lat:.6f}")
            ...
            if self.is_search_paused:
                ...
                self._safe_update_status(f"搜索完成，共 {len(self.valid_locations)} 个有效坐标", "green")
    ...
    def toggle_search_pause(self):
        ...
        if self.is_search_paused:
            ...
            self.update_api_response("⏸️ 正在暂停搜索...")
    
    def get_distance_config_from_ui(self):
        """从UI输入框获取距离配置"""
        try:
            # 相邻点最小距离（米转公里）
            adj_min = float(self.adj_min_input.text().strip() or "500") / 1000
            # 相邻点最大距离（米转公里）
            adj_max = float(self.adj_max_input.text().strip() or "1000") / 1000
            # 非相邻点最小距离（米转公里）
            non_adj_min = float(self.non_adj_min_input.text().strip() or "500") / 1000
    ...
    def select_optimal_waypoints(self, start_point, end_point, waypoint_num, used_waypoints_set):
        """智能选择最优的途径点
        约束条件：
        1. 相邻点（包含起终点）导航距离在用户设置范围内（默认500米-1000米）
        2. 不相邻的点导航距离大于用户设置值（默认500米）
        """
        # 从UI获取距离配置
        ...
        # 距离约束参数（从UI获取）
        min_adj_km = config.get('between_waypoint_min', 0.5)  # 相邻点最小距离
        max_adj_km = config.get('between_waypoint_max', 1.0)  # 相邻点最大距离
        non_adj_min = config.get('non_adjacent_min', 0.5)     # 不相邻点最小距离
        
        self.update_api_response(f"📊 距离约束: 相邻点{min_adj_km*1000:.0f}m-{max_adj_km*1000:.0f}m, 不相邻点>{non_adj_min*1000:.0f}m")
        ...
        for candidate in candidates:
            candidate['dist_to_start'] = self.calculate_distance_between_points(candidate, start_point)
            candidate['dist_to_end'] = self.calculate_distance_between_points(candidate, end_point)
        
        # 按与起点距离排序，保证途径点顺序从起点到终点
        candidates.sort(key=lambda p: p['dist_to_start'])
        
        selected_waypoints = []
        
        # 第一个途径点需要与起点距离在范围内
        self.update_api_response(f"🔍 开始验证途径点距离...")
        for candidate in candidates:
            # 获取与起点的距离
            dist_to_start = calc_distance(start_point, candidate)
            candidate['calc_dist_to_start'] = dist_to_start
            if min_adj_km <= dist_to_start <= max_adj_km:
                selected_waypoints.append(candidate)
                self.update_api_response(f"   ✅ 选中第1个途径点: {candidate['name']} (距起点{dist_to_start*1000:.0f}m)")
                break
            else:
                self.update_api_response(f"   ❌ 跳过 {candidate['name']}: 距起点{dist_to_start*1000:.0f}m 不在{min_adj_km*1000:.0f}-{max_adj_km*1000:.0f}m范围")
        
        if not selected_waypoints:
            # 如果没有找到符合条件的第一个点，放宽条件选择最近的
            if candidates:
                # 选择距离最接近目标范围的点
                ideal_dist = (min_adj_km + max_adj_km) / 2
                best_candidate = min(candidates, key=lambda c: abs(c.get('calc_dist_to_start', c['dist_to_start']) - ideal_dist))
                selected_waypoints.append(best_candidate)
                dist = best_candidate.get('calc_dist_to_start', best_candidate['dist_to_start'])
                self.update_api_response(f"   ⚠️ 放宽条件选中第1个途径点: {best_candidate['name']} (距离{dist*1000:.0f}m)")
        
        # 选择剩余途径点
        while len(selected_waypoints) < waypoint_num and len(candidates) > len(selected_waypoints):
            last_point = selected_waypoints[-1]
            best_candidate = None
            best_score = -1
            
            for candidate in candidates:
                if candidate in selected_waypoints:
                    continue
                
                # 获取与上一个点的距离（相邻点约束）
                dist_to_prev = calc_distance(last_point, candidate)
                if not (min_adj_km <= dist_to_prev <= max_adj_km):
                    continue
                
                # 检查与所有已选点的距离（不相邻点约束）
                valid_non_adjacent = True
                for i, wp in enumerate(selected_waypoints[:-1]):  # 排除最后一个（已检查相邻）
                    dist_to_wp = calc_distance(wp, candidate)
                    if dist_to_wp < non_adj_min:
                        valid_non_adjacent = False
                        self.update_api_response(f"   ❌ {candidate['name']}: 与{wp['name']}距离{dist_to_wp*1000:.0f}m < {non_adj_min*1000:.0f}m")
                        break
                
                if not valid_non_adjacent:
                    continue
                
                # 检查与起点的距离（不相邻约束）
                dist_to_start = calc_distance(start_point, candidate)
                if dist_to_start < non_adj_min and candidate != selected_waypoints[0]:
                    continue
                
                # 计算评分：优先选择距离上一个点适中的候选点
                ideal_dist = (min_adj_km + max_adj_km) / 2
                score = 1 - abs(dist_to_prev - ideal_dist) / max_adj_km
                
                if score > best_score:
                    best_score = score
                    best_candidate = candidate
                    best_candidate['_calc_dist_to_prev'] = dist_to_prev
            
            if best_candidate:
                selected_waypoints.append(best_candidate)
                dist_to_prev = best_candidate.get('_calc_dist_to_prev', 0)
                self.update_api_response(f"   ✅ 选中第{len(selected_waypoints)}个途径点: {best_candidate['name']} (距上一点{dist_to_prev*1000:.0f}m)")
            else:
                # 放宽条件，从剩余候选中选择距离最近的
                remaining = [c for c in candidates if c not in selected_waypoints]
                if remaining:
                    # 按直线距离排序
                    remaining.sort(key=lambda p: self.calculate_distance_between_points(p, last_point))
                    for candidate in remaining[:5]:  # 只检查前5个
                        dist_to_prev = calc_distance(last_point, candidate)
                        # 放宽到只检查最小距离
                        if dist_to_prev >= non_adj_min:
                            selected_waypoints.append(candidate)
                            self.update_api_response(f"   ⚠️ 放宽条件选中第{len(selected_waypoints)}个途径点: {candidate['name']} (距离{dist_to_prev*1000:.0f}m)")
                            break
                    else:
                        self.update_api_response(f"   ❌ 无法找到更多符合条件的途径点")
                        break
                else:
                    break
        ...
        if selected_waypoints:
            ...
            if dist_to_end < min_adj_km or dist_to_end > max_adj_km:
                self.update_api_response(f"   ⚠️ 最后一个途径点距终点{dist_to_end*1000:.0f}m，不在{min_adj_km*1000:.0f}-{max_adj_km*1000:.0f}m范围内")
            else:
                self.update_api_response(f"   ✅ 最后一个途径点距终点{dist_to_end*1000:.0f}m")
        
        self.update_api_response(f"📌 最终选择的途径点: {len(selected_waypoints)}个")
        
        # 如果途径点数量不足，记录警告
        if len(selected_waypoints) < waypoint_num:
            self.update_api_response(f"⚠️ 警告：只找到 {len(selected_waypoints)}/{waypoint_num} 个符合条件的途径点")
        ...
        for key_index, current_key in enumerate(keys_to_try):
            try:
                ...
                if data.get('status') == '1' and data.get('route'):
                    ...
                    if points:
                        self.update_api_response(f"✅ 使用密钥{key_index + 1}成功获取驾驶路线，共{len(points)}个坐标点")
                        return points
                else:
    ...
    def generate_route(self, route_num, waypoint_num, existing_routes=None):
        """【改进版】生成一条测试路线
        支持目标里程规划：根据用户设置的目标里程和偏差范围选择合适的起终点
        """
        ...
        if distance_range:
            # 有目标里程要求，需要选择符合距离的起终点
            min_dist, max_dist = distance_range
            self.update_api_response(f"🎯 目标里程范围: {min_dist:.1f}km - {max_dist:.1f}km")
            
            # 收集所有符合距离要求的起终点组合
            # 注意：直线距离通常小于实际导航距离，假设导航距离约为直线距离的1.3-1.5倍
            # 所以直线距离应在 min_dist/1.5 到 max_dist/1.3 范围内
            straight_min = min_dist / 1.5
            straight_max = max_dist / 1.2
            
            valid_pairs = []
            for i, loc1 in enumerate(self.valid_locations):
                for j, loc2 in enumerate(self.valid_locations):
                    if i >= j:
                        continue
                    dist = self.calculate_distance_between_points(loc1, loc2)
                    if straight_min <= dist <= straight_max:
                        valid_pairs.append((loc1, loc2, dist))
            
            if valid_pairs:
                # 随机选择一对
                random.shuffle(valid_pairs)
                start_point, end_point, straight_distance = valid_pairs[0]
                self.update_api_response(f"✅ 找到 {len(valid_pairs)} 对符合目标里程的起终点")
            else:
                # 没有符合的，选择最接近目标的
                self.update_api_response(f"⚠️ 没有完全符合目标里程的起终点，选择最接近的")
                target_straight = (straight_min + straight_max) / 2
                best_pair = None
                best_diff = float('inf')
                ...
                for i, loc1 in enumerate(self.valid_locations):
                    for j, loc2 in enumerate(self.valid_locations):
                        if i >= j:
                            continue
                        dist = self.calculate_distance_between_points(loc1, loc2)
                        diff = abs(dist - target_straight)
                        if diff < best_diff:
                            best_diff = diff
                            best_pair = (loc1, loc2, dist)
                
                if best_pair:
                    start_point, end_point, straight_distance = best_pair
        
        # 如果没有目标里程或未找到合适的，随机选择
        ...
        waypoints = []
        if waypoint_num > 0:
            # 多次尝试选择途径点，如果不足则重新选择起终点
            max_waypoint_attempts = 5
            for wp_attempt in range(max_waypoint_attempts):
                ...
                if len(waypoints) >= waypoint_num:
                    break
                elif wp_attempt < max_waypoint_attempts - 1:
                    # 途径点不足，重新选择起终点
                    self.update_api_response(f"⚠️ 途径点不足，尝试重新选择起终点 (尝试{wp_attempt + 2}/{max_waypoint_attempts})")
                    # 重新随机选择起终点
                    for _ in range(20):
                        new_start_idx = random.randint(0, len(self.valid_locations) - 1)
                        new_end_idx = random.randint(0, len(self.valid_locations) - 1)
                        if new_start_idx != new_end_idx:
                            start_point = self.valid_locations[new_start_idx]
                            end_point = self.valid_locations[new_end_idx]
                            straight_distance = self.calculate_distance_between_points(start_point, end_point)
                            self.update_api_response(
                                f"📍 重新选择: 起点[{start_point['name']}] → 终点[{end_point['name']}] "
                                f"(直线距离: {straight_distance:.2f}km)"
                            )
                            break
        ...
        if waypoints:
            config = self.route_config
            min_adj_km = config.get('between_waypoint_min', 0.5)
            max_adj_km = config.get('between_waypoint_max', 1.5)
            non_adj_min = config.get('non_adjacent_min', 0.5)
            
            all_points = [start_point] + waypoints + [end_point]
            distance_valid = True
            
            self.update_api_response(f"📏 验证途径点距离...")
            for i in range(len(all_points) - 1):
                dist = self.calculate_distance_between_points(all_points[i], all_points[i+1])
                self.update_api_response(f"   {all_points[i]['name']} → {all_points[i+1]['name']}: {dist:.3f}km")
                if dist < min_adj_km or dist > max_adj_km:
                    self.update_api_response(f"   ⚠️ 相邻点距离不在{min_adj_km}-{max_adj_km}km范围内")
                    # 不直接返回None，继续生成但标记警告
            ...
            for i in range(len(all_points)):
                for j in range(i + 2, len(all_points)):
                    ...
                    if dist < non_adj_min:
                        self.update_api_response(f"   ⚠️ 不相邻点 {all_points[i]['name']} 和 {all_points[j]['name']} 距离{dist:.3f}km < {non_adj_min}km")
    ...
    def _open_all_nav_urls(self):
        ...
        reply = QMessageBox.question(self, "确认", f"确定要打开全部 {len(urls)} 条链接吗？",
                                      QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
        if reply == QMessageBox.Yes:
