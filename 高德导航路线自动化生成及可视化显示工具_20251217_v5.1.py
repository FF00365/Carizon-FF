...
class SettingsDialog(QDialog):
    ...
    def init_ui(self):
        ...
        adj_label.setStyleSheet("font-size: 18px;")
        ...
        self.adj_min_input.setStyleSheet("font-size: 16px;")
        adj_layout.addWidget(self.adj_min_input)
        
        separator_label = QLabel(" - ")
        separator_label.setStyleSheet("font-size: 18px;")
        ...
        self.adj_max_input.setStyleSheet("font-size: 16px;")
        ...
        target_label.setStyleSheet("font-size: 18px;")
        ...
        self.target_distance_input.setStyleSheet("font-size: 16px;")
        form_layout.addWidget(self.target_distance_input, row, 1)
        
        # 偏差范围设置
        row += 1
        tolerance_label = QLabel("偏差范围(km):")
        tolerance_label.setStyleSheet("font-size: 18px;")
        ...
        self.distance_tolerance_input.setStyleSheet("font-size: 16px;")
        form_layout.addWidget(self.distance_tolerance_input, row, 1)
        
        # 地点筛选距离
        row += 1
        filter_label = QLabel("地点筛选距离(m):")
        filter_label.setStyleSheet("font-size: 18px;")
        ...
        self.location_filter_input.setStyleSheet("font-size: 16px;")
        form_layout.addWidget(self.location_filter_input, row, 1)
        
        # 坐标纠偏开关
        row += 1
        rectify_label = QLabel("坐标纠偏:")
        rectify_label.setStyleSheet("font-size: 18px;")
        form_layout.addWidget(rectify_label, row, 0)
        
        self.rectify_checkbox = QCheckBox("启用坐标纠偏（修正到最近公开道路）")
        self.rectify_checkbox.setStyleSheet("font-size: 16px;")
        form_layout.addWidget(self.rectify_checkbox, row, 1)
        
        layout.addLayout(form_layout)
        
        # 说明文字
        note_label = QLabel("💡 提示：以上设置为选填项，留空表示不限制")
        note_label.setStyleSheet("font-size: 14px; color: #666;")
...
class MainWindow(QMainWindow):
    ...
    def create_auto_route_tab(self):
        ...
        self.distance_calc_combo.setFixedWidth(220)
        ...
        self.spatial_sort_combo.setStyleSheet("font-size: 18px;")
    ...
    def select_optimal_waypoints(self, start_point, end_point, waypoint_num, used_waypoints_set, moving_left=True):
        """智能选择最优的途径点 - 空间排序 + 贪心算法
        
        算法流程：
        1. 空间排序：根据用户选择的算法对候选点进行空间排序
        2. 贪心优化：从起点出发，每次选择最近的未访问点，压缩路线长度
        3. 距离约束：检查相邻点和非相邻点的距离约束
        """
        # 从UI获取距离配置
        ...
        for i, wp in enumerate(selected_waypoints):
            ...
            self.update_api_response(f"   ✅ 第{i+1}个途径点: {wp['name']} (距{dist*1000:.0f}m)")
            current_point = wp
    ...
    def generate_route(self, route_num, waypoint_num, existing_routes=None):
        ...
        起点设置（全局统一方向）：
        1. 单条路线起点：从当前路线的所有原始点中，选"最左上角点"
           —— 先比较经度（选经度最小→最西），若经度一致，选纬度最大→最北的点
        2. 多路线串联起点：第一条路线按上述规则选起点；后续每条路线的起点，
           直接承接上一条路线的终点，避免全局绕路
        """
        ...
        if existing_routes:
            ...
            # 第一条路线：选择"最左上角点"
            # 先比较经度（选最小→最西），若经度一致，选纬度最大→最北
            start_point = self.find_leftmost_top_point(self.valid_locations)
            self.update_api_response(f"🚩 路线 {route_num}: 从最左上角点开始 ({start_point['name']})")
        ...
        # 选择终点：使用贪心算法，选择离排序后最后一个点最近的点作为终点
        # 或根据目标里程选择合适的终点
        if distance_range:
            ...
            # 在可用点中找符合距离的
            valid_endpoints = []
            for p in sorted_available:
                dist = self.calculate_distance_between_points(start_point, p)
                if straight_min <= dist <= straight_max:
                    valid_endpoints.append((p, dist))
            
            if valid_endpoints:
                # 选择距离起点适中的点作为终点
                end_point, straight_distance = valid_endpoints[-1]  # 选最后一个（排序后的远点）
            else:
                # 没有符合距离的，选择距离最接近目标的点
                target_straight = (straight_min + straight_max) / 2
                best_endpoint = min(sorted_available, 
                                   key=lambda p: abs(self.calculate_distance_between_points(start_point, p) - target_straight))
                end_point = best_endpoint
                straight_distance = self.calculate_distance_between_points(start_point, end_point)
        else:
            # 没有目标里程，选择排序后的最后一个点作为终点（贪心思想：遍历所有点）
            end_point = sorted_available[-1] if sorted_available else available_points[0]
            straight_distance = self.calculate_distance_between_points(start_point, end_point)
        ...
        if waypoint_num > 0:
            ...
            waypoints = self.select_optimal_waypoints(
                start_point, end_point, waypoint_num, used_points_set
            )
